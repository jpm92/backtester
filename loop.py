...
from data import HistoricCSVDataHandler, DataSource  # QuandlDataHandler
# from strategies.hold import BuyAndHoldStrategy, SellAndHoldStrategy
from strategies.macd import MovingAveragesLongStrategy
...
def backtest(events, data, portfolio, strategy, broker):
    while True:
        ...
        if data.continue_backtest is False:
            break
...
#             events, 'csv/', ['OMXS30'], DataSource.NASDAQ)
...
#         # strategy = MovingAveragesLongShortStrategy(
#         #     data, events, portfolio, 100, 200, version=1)
#         # strategy = MovingAveragesMomentumStrategy(
#         #     data, events, portfolio, 100, 200)
#         # strategy = StopLossStrategy(data, events, portfolio, 0.9)
#         # strategy = DivideAndConquerStrategy(data, events, portfolio)
#         strategy = MovingAveragesLongStrategy(
#             data,
#             events,
#             portfolio,
#             s,
...
portfolio = NaivePortfolio(data, events, '', initial_capital=2000)
strategy = MovingAveragesLongStrategy(
    data, events, portfolio, 50, 100, version=1
)
portfolio.strategy_name = strategy.name
broker = SimulateExecutionHandler(events)
