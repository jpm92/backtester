...
class MovingAveragesLongStrategy(Strategy):
    ...
    def calculate_long_short(self, df):
        ...
        price_long = None
        if self.version == 1:
            price_short = df['Close'].ewm(span=self.short_period,
                                          min_periods=self.short_period,
                                          adjust=False).mean()[-1]
            price_long = df['Close'].ewm(span=self.long_period,
                                         min_periods=self.long_period,
                                         adjust=False).mean()[-1]
        else:
            price_short = df['Close'].tail(self.long_period).ewm(
                span=self.short_period,
                adjust=False
            ).mean()[-1]
            price_long = df['Close'].tail(
                self.long_period
                ).ewm(span=self.long_period, adjust=False).mean()[-1]
                ...
                if data is not None and len(data) >= self.long_period:
                    ...
                    self.strategy[symbol] = self.strategy[symbol].append(
                        {'Date': date,
                         ...
                         'Long': price_long},
                        ignore_index=True)
                    if (self.bought[symbol] is False and
                            price_short > price_long):
                        quantity = math.floor(
                            self.portfolio.current_holdings['cash'] / price)
                        signal = SignalEvent(symbol, date, 'LONG', quantity)
                        self.events.put(signal)
                        self.bought[symbol] = True
                        self.signals[symbol] = self.signals[symbol].append(
                            {'Signal': quantity,
                             'Date': date},
                            ignore_index=True)
                        if self.verbose:
                            print("Long", date, price)
                    elif (self.bought[symbol] is True and
                            price_short < price_long):
    ...
    def plot(self):
        ...
        for symbol in self.symbol_list:
            ...
            df.columns = ['OMXS30']
            # df['Short'] = df['OMXS30'].ewm(
            #     span=self.short_period,
            #     min_periods=self.short_period,
            #     adjust=False).mean()
            # df['Long'] = df['OMXS30'].ewm(
            #     span=self.long_period,
            #     min_periods=self.long_period,
            #     adjust=False).mean()
            ...
            strategy_ax.plot(
                self.strategy[symbol]['Short'],
                label='Short EMA',
                color='grey')
            strategy_ax.plot(
                self.strategy[symbol]['Long'], label='Long EMA', color='k')
            strategy_ax.plot(
                short_index, df['OMXS30'].loc[short_index], 'v',
                markersize=10, color='r', label='Exit')
            strategy_ax.plot(
                long_index, df['OMXS30'].loc[long_index], '^',
                markersize=10, color='g', label='Long')
...
class MovingAveragesLongShortStrategy(Strategy):
    ...
    def calculate_long_short(self, df):
        ...
        price_long = None
        if self.version == 1:
            price_short = df['Close'].ewm(
                span=self.short_period,
                min_periods=self.short_period,
                adjust=False).mean()[-1]
            price_long = df['Close'].ewm(
                span=self.long_period,
                min_periods=self.long_period,
                adjust=False).mean()[-1]
        else:
            price_short = df['Close'].tail(self.long_period).ewm(
                span=self.short_period, adjust=False).mean()[-1]
            price_long = df['Close'].tail(self.long_period).ewm(
                span=self.long_period, adjust=False).mean()[-1]
                    ...
                    if (self.bought[symbol] is False and
                        ...
                        quantity = math.floor(
                            self.portfolio.current_holdings[
                                'cash'] / price + current_positions)
                        signal = SignalEvent(symbol, date, 'EXIT', math.fabs(
                            current_positions)
                            )
                        self.events.put(signal)
                        signal = SignalEvent(symbol, date, 'LONG', quantity)
                        self.events.put(signal)
                        self.bought[symbol] = True
                        self.signals[symbol] = self.signals[
                            symbol
                            ].append(
                                {'Signal': quantity, 'Date': date},
                                ignore_index=True
    ...
    def plot(self):
        ...
        for symbol in self.symbol_list:
            ...
            strategy_ax.plot(
                ...
                label='Short EMA',
                color='grey')
            strategy_ax.plot(self.strategy[symbol]['Long'],
                             label='Long EMA', color='k')
            strategy_ax.plot(short_index,
                             df['OMXS30'].loc[short_index],
                             'v',
                             markersize=10,
                             color='r',
                             label='Short')
            strategy_ax.plot(long_index,
                             df['OMXS30'].loc[long_index],
                             '^',
                             markersize=10,
                             color='g',
                             label='Long')
            ...
            for symbol in self.symbol_list:
                ...
                if data is not None and len(data) >= self.long_period:
                    ...
                    if price_short >= price_long:
                        quantity = math.floor(
                            factor * self.portfolio.current_holdings[
                                'cash'] / price)
                        if quantity != 0:
                            signal = SignalEvent(
                                symbol, date, 'LONG', quantity)
                            self.events.put(signal)
                            if self.verbose:
                                print('Long', date, price)
                    else:
                        quantity = math.floor(
                            factor/2 * self.portfolio.current_positions[
                                symbol])
                        if quantity != 0:
                            signal = SignalEvent(
                                symbol, date, 'SHORT', quantity)
                            self.events.put(signal)
                            if self.verbose:
                                print('Short', date, price)
